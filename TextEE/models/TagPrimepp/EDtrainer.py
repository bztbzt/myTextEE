...
from .EDmodel import TagPrimeppEDModel
from scorer import compute_ED_scores, print_scores
import ipdb

logger = logging.getLogger(__name__)

EDBatch_fields = ['batch_doc_id', 'batch_wnd_id', 'batch_tokens', 'batch_pieces', 'batch_token_lens', 'batch_token_num', 'batch_text', 'batch_triggers', 'batch_event_type']
...
class TagPrimeppEDTrainer(BasicTrainer):
    ...
    def load_model(self, checkpoint=None):
        if checkpoint:
            ...
            self.model = TagPrimeppEDModel(self.config, self.tokenizer, self.type_set)
    ...
    def process_data_for_training(self, data):
        ...
        for dt in data:
            ...
            for trigger in dt["triggers"]:
                ...
                if trigger[2] not in no_overlap_flags:
                    no_overlap_flags[trigger[2]] = np.ones((len(dt["tokens"]), ), dtype=bool)
                if np.all(no_overlap_flags[trigger[2]][start:end]):
                    new_triggers.append(trigger)
                    no_overlap_flags[trigger[2]][start:end] = False
                            
            pieces = [self.tokenizer.tokenize(t, is_split_into_words=True) for t in dt["tokens"]]
            token_lens = [len(p) for p in pieces]
            
            pos_event_types = set(t[2] for t in new_triggers)
            neg_event_types = [event_type for event_type in self.type_set["trigger"] if event_type not in pos_event_types]
            np.random.shuffle(neg_event_types)
            neg_event_types = neg_event_types[:self.config.n_negative]
            
            for event_type in sorted(pos_event_types)+sorted(neg_event_types):
                new_triggers_ = [t for t in new_triggers if t[2] == event_type]
    ...
    def predict(self, data, **kwargs):
        ...
        internal_data = self.process_data_for_testing(data)
