...
from .EAEmodel import CRFTaggingGNNEAEModel
from scorer import compute_EAE_scores, print_scores
import ipdb

logger = logging.getLogger(__name__)

EAEBatch_fields = ['batch_doc_id', 'batch_wnd_id', 'batch_tokens', 'batch_pieces', 'batch_token_lens', 'batch_token_num', 'batch_text', 'batch_trigger', 'batch_arguments','batch_adj']
...
def EAE_collate_fn(batch):
    return EAEBatch(
        ...
        batch_adj = [instance["adj"] for instance in batch], 
    )

class CRFTaggingGNNEAETrainer(BasicTrainer):
    ...
    def load_model(self, checkpoint=None):
        if checkpoint:
            ...
            self.model = CRFTaggingGNNEAEModel(self.config, self.tokenizer, self.type_set)
    ...
    def train(self, train_data, dev_data, **kwargs):
        ...
        for epoch in range(1, self.config.max_epoch+1):
            ...
            print(f'best_epoch: {best_epoch}, best_scores: {best_scores}')
